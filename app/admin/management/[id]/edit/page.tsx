...
export default function GrantEditPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  useEffect(() => {
    ...
    getSubsidyDetail(supabase, id)
      .then((detail) => {
        if (detail) {
          setSubsidyName(detail.title);
          setImplementingAgency(detail.implementingOrganization === '未設定' ? '' : detail.implementingOrganization);
          setRegion(detail.region === '全国' ? '全国' : detail.region || '選択してください');
          setAmount(detail.grantAmount === '未設定' ? '' : detail.grantAmount);
          // Convert "~2025年3月31日" to "2025年3月31日"
          const period = detail.applicationPeriod;
          const deadlineValue = period.startsWith('~') ? period.substring(1) : period;
          setDeadline(deadlineValue === '未設定' ? '' : deadlineValue);
          setSubsidyRate(detail.subsidyRate === '未設定' ? '' : detail.subsidyRate);
          setOverview(detail.overview);
          setEligibleActivities(detail.eligibleActivities || []);
          setEligibilityConditions(detail.eligibilityConditions || []);
          setRequiredDocuments(detail.requiredDocuments || []);
  ...
  const isFormValid = subsidyName.trim() && overview.trim();
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/admin/subsidies/${id}`, {
        method: 'PUT',
              ...
              <div className="flex flex-col sm:flex-row gap-4 w-full">
                ...
                <FormField label="地域" required className="flex-1">
                  <div className="relative">
                    ...
                    >
                      {REGION_OPTIONS.map((option: string) => (
    ...
    <div className="flex flex-col gap-2 w-full">
      ...
      <div className="flex flex-col gap-2">
        ...
        <button
          ...
          className="text-sm font-normal leading-5 tracking-[-0.150390625px] text-[#9810FA] hover:underline w-fit"
        >
          {addButtonText}
        </button>
        {items.length > 0 && (
